...
public class ForFile {
     ...
     * 
     * */
    public static boolean writeFileContinu_Chixu( ArrayList<String> strlist) throws IOException{
    	Boolean bool = false;
    	try {  
            ...
            // 创建文件夹  
            File tf = new File("E:\\临时txt文件");  
            if (!tf.exists()) {  
                tf.mkdir();  
                System.err.println(tf + " 创建OK");  
            }  
            File f = new File("E:\\临时txt文件\\outfile"+ sdf2.format(new Date()) + ".txt");  
            if (!f.exists()) {  
                f.createNewFile();  
